...
function PropertyCard({ property }) {
  ...
  return (
    ...
    >
      ...
      <Link to={`/immobilien/${property.id}`} className="block relative overflow-hidden" style={{ height: 220 }}>
        <img
        ...
        <div className="absolute top-3 left-3 flex flex-wrap gap-1.5">
          {property.tags.slice(0, 2).map(tag => (
            <span key={tag} className="font-nunito font-700 text-xs px-2.5 py-1 rounded-full bg-white text-gray-700"
              style={{ boxShadow: '0 1px 6px rgba(0,0,0,0.12)' }}>{tag}</span>
          ))}
        </div>
        <button
        ...
        >
          {liked ? <FaHeart size={15} color="#ef4444" /> : <FaRegHeart size={15} color="#aaa" />}
        </button>
      ...
      {/* Content */}
      <div className="p-5">
        <p className="font-nunito text-xs text-gray-400 mb-1">📍 {property.location}</p>
        <Link to={`/immobilien/${property.id}`}>
          <h3 className="font-playfair text-lg text-gray-900 mb-3 leading-snug hover:underline" style={{ fontWeight: 600 }}>
            {property.title}
          </h3>
        </Link>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="font-playfair text-2xl text-gray-900" style={{ fontWeight: 700 }}>
          ...
          <Link to={`/immobilien/${property.id}`}>
            <motion.span
              className="font-nunito font-700 text-xs px-4 py-2.5 rounded-xl text-white inline-block"
    ...
    <div className="min-h-screen" style={{ background: 'var(--site-bg, #FAFAFA)' }}>
      ...
      <div className="pt-28 pb-10 px-8 md:px-16" style={{ background: 'white', borderBottom: '1px solid #eee' }}>
        <div className="max-w-7xl mx-auto">
          ...
          <h1 className="font-playfair text-4xl md:text-5xl text-gray-900 mt-2 mb-2" style={{ fontWeight: 700 }}>Immobilien</h1>
      ...
      <div className="sticky top-[72px] z-40 px-8 md:px-16 py-4"
        ...
        <div className="max-w-7xl mx-auto flex flex-wrap items-center gap-3">
          <div className="flex rounded-xl overflow-hidden" style={{ border: '1px solid #e5e7eb' }}>
            {['kaufen', 'mieten'].map(d => (
              <button key={d} onClick={() => switchDeal(d)}
                ...
                style={{ background: deal === d ? 'var(--site-btn, #1e1a16)' : 'white', color: deal === d ? 'white' : '#555' }}>
          ...
          <Select value={priceIdx} onChange={setPriceIdx} options={priceRanges.map(r => ({ label: r.label, value: r.label }))} />
      ...
      <div className="max-w-7xl mx-auto px-8 md:px-16 py-10">
        <AnimatePresence mode="popLayout">
          {filtered.length > 0 ? (
            ...
            <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              <p className="font-playfair text-2xl text-gray-300 mb-2" style={{ fontWeight: 600 }}>Keine Objekte gefunden</p>
        ...
        {filtered.length > 0 && (
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            ...
            <h2 className="font-playfair text-3xl mb-3" style={{ fontWeight: 700 }}>Nichts Passendes gefunden?</h2>
