...
function StatItem({ target, suffix, label, decimals = 0, duration = 1600 }) {
  ...
  return (
    <div ref={ref}>
      <p className="font-pacifico text-2xl" style={{ background: 'linear-gradient(135deg,#FFB5D8,#C5B5EA)', WebkitBackgroundClip: 'text', backgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
...
export default function Hero() {
  ...
  return (
    <section id="home" className="relative min-h-screen flex items-center overflow-hidden" style={{ background: '#ffffff', paddingTop: isAdmin ? 52 : 0 }}>
      <div className="absolute inset-0 pointer-events-none" style={{ background: 'radial-gradient(ellipse 70% 80% at 80% 50%, #FFF5F9 0%, #ffffff 55%)' }} />
      <div className="absolute inset-0 pointer-events-none" style={{ background: 'radial-gradient(ellipse 50% 60% at 10% 40%, #F8F4FF 0%, transparent 60%)' }} />
      ...
      <div className="relative max-w-7xl mx-auto px-2 md:px-4 py-24 w-full">
        <div className="max-w-md">
          <motion.div {...stagger(0.1)} className="mb-6">
            <div className="inline-flex items-center gap-2 px-5 py-2 rounded-full" style={{ background: 'rgba(255,181,216,0.15)', border: '1px solid rgba(255,181,216,0.35)' }}>
              <span className="text-sm">🐾</span>
              <span className="font-nunito font-700 text-xs tracking-[0.25em] uppercase" style={{ color: '#d4789a' }}>Premium Hundesalon · Pforzheim</span>
            </div>
          </motion.div>
          ...
          <motion.div {...stagger(0.52)} className="flex flex-wrap gap-2 mb-8">
            {['Zertifizierte Groomerin', 'Naturprodukte', 'Kleine & Mittlere Rassen'].map(chip => (
              <span key={chip} className="font-nunito font-600 text-xs px-3.5 py-1.5 rounded-full" style={{ background: '#f5f0ff', border: '1px solid #e8dff5', color: '#9b85c4' }}>{chip}</span>
            ))}
          </motion.div>

          <motion.div {...stagger(0.62)} className="flex flex-wrap items-center gap-3 mb-9">
            <div className="flex items-center gap-2.5 px-4 py-2.5 rounded-2xl" style={{ background: 'white', border: '1px solid #ece8f5', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
          ...
          <motion.div {...stagger(0.9)} className="flex gap-8 mt-10 pt-8" style={{ borderTop: '1px solid #ede8f5' }}>
      ...
      <motion.div className="md:hidden w-full px-6 pb-10" initial={{ opacity: 0, y: 20 }} animate={ready ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.5, duration: 0.8 }}>
        <div className="rounded-3xl overflow-hidden w-full" style={{ aspectRatio: '4/3', boxShadow: '0 12px 40px rgba(255,181,216,0.25)' }}>
          <EditableImage path="hero.image" src={hero.image} alt="Hero" className="w-full h-full object-cover"
      ...
      <motion.div className="absolute hidden md:flex items-center gap-2 px-4 py-2.5 rounded-2xl"
        style={{ right: '6%', top: '22%', background: 'rgba(255,255,255,0.85)', backdropFilter: 'blur(16px)', border: '1px solid rgba(255,181,216,0.25)', boxShadow: '0 8px 32px rgba(0,0,0,0.08)' }}
        ...
        <motion.div animate={{ y: [0, -5, 0] }} transition={{ repeat: Infinity, duration: 3.5, ease: 'easeInOut' }} className="flex items-center gap-2">
          ...
          <div><p className="font-nunito font-800 text-xs" style={{ color: '#1a1025' }}>Profi-Styling</p><p className="font-nunito text-xs" style={{ color: '#b0a0c0' }}>Alle Rassen</p></div>
        </motion.div>
      </motion.div>
      <motion.div className="absolute hidden md:flex items-center gap-2 px-4 py-2.5 rounded-2xl"
        style={{ right: '10%', bottom: '20%', background: 'rgba(255,255,255,0.85)', backdropFilter: 'blur(16px)', border: '1px solid rgba(197,181,234,0.3)', boxShadow: '0 8px 32px rgba(0,0,0,0.08)' }}
        ...
        <motion.div animate={{ y: [0, 5, 0] }} transition={{ repeat: Infinity, duration: 4, ease: 'easeInOut', delay: 1 }} className="flex items-center gap-2">
          ...
          <div><p className="font-nunito font-800 text-xs" style={{ color: '#1a1025' }}>Naturprodukte</p><p className="font-nunito text-xs" style={{ color: '#b0a0c0' }}>Sanft & sicher</p></div>
        </motion.div>
      </motion.div>
    </section>
  )
